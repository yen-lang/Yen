...
// ---------------- Patterns ----------------
// Base Pattern class for pattern matching
struct Pattern {
...
// Variable binding pattern: x (binds value to x)
...
struct LetStmt : Statement {
    std::string name;
    std::unique_ptr<Expression> expression;
    std::optional<std::string> typeAnnotation; // Optional type: let x: int32 = 10;
...
struct ConstStmt : Statement {
    std::string name;
    std::unique_ptr<Expression> expression;
    std::string typeAnnotation;  // Required for const
...
struct FunctionStmt : Statement {
    std::string name;
    std::vector<std::string> parameters;
    std::vector<std::string> parameterTypes; // Optional: ["int32", "float64"]
    std::string returnType; // Optional: "int32"
...
// External function declaration (inside extern block)
struct ExternFunctionDecl {
    std::string name;
    std::vector<std::string> parameters;
    std::vector<std::string> parameterTypes;
    std::string returnType;
    bool isVarArg; // Support for ... (varargs)
...
struct AssertStmt : Statement {
    std::unique_ptr<Expression> condition;
    std::string message;
    bool isDebugOnly;  // true for debug_assert
